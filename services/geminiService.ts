...
const HARDCODED_API_KEY = "AIzaSyD9ZK0a92__91N5RgXCCk1tEWDNAU4tZZ8"; // From user screenshot
const API_KEY = HARDCODED_API_KEY || (window as any).GEMINI_API_KEY || import.meta.env.VITE_GEMINI_API_KEY || (typeof process !== 'undefined' && (process.env.VITE_GEMINI_API_KEY || process.env.GEMINI_API_KEY));

if (!API_KEY) {
    console.warn("⚠️ Gemini API key not found. AI analysis will be mocked.");
} else {
    console.log("💎 Gemini API Key detected (length: " + API_KEY.length + "). Ready for analysis.");
}
